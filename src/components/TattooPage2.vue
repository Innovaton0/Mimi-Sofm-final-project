<template>
  ...
  <div v-if="images" class="flex flex-wrap gap-3 mx-6 pb-10">
    <div v-for="(image, index) in images" :key="index" @click="updateBigImage(image.src.medium)" class="flex-[2_2_200px]">
      <img v-if="image.src.medium" :src="image.src.medium" alt="tattoo" class="cursor-pointer w-full h-44 object-cover" />
    </div>
  </div>
      ...
      <div @click.stop class="bg-white mx-8 p-8 shadow-2xl w-[40rem] lg:w-[54rem] flex justify-center gap-4">
        <div class="py-2">
          <div class="text-sm flex items-center justify-end gap-1 mb-1">
            <p class="text-xs lg:text-sm">Guardar como favorito</p>
            <i class="fa-solid fa-heart hover:text-[#F09235] active:text-[#F09235]"></i>
          </div>
        ...
        <div class="flex flex-col items-center w-[21rem] px-2 justify-around border-l border-l-black">
          <p class="text-center leading-2 text-lg md:text-xl lg:text-2xl">Conoce al <router-link to="/tattooartists/:id" class="font-bold text-[#F09235] hover:text-[#651c19]">artista</router-link> que hizo este tatuaje</p>
          <router-link to="/tattooartists/:id">
            <img src="../assets/Tatuajes Home/pexels-photo-1433273.png" alt="Imagen Redonda" class="w-24 h-24 lg:w-40 lg:h-40 p-2 rounded-full object-cover" />
          </router-link>
          <ul class="flex gap-4">
